...
FW_BIN = "apex_latest_single_ep.bin"
TF_MODEL = "inception_v4_299_quant_edgetpu.tflite"
...
def get_image_pixels(image_data):
  """
  Parse image file
  """
...
def firmware_check():
  """
  check which device state, we are dealing with and return the device
  """
  # for windows, set the backend to use usbdk?
  ...
  if dev is None:
    ...
    for i in range(0, len(fw), 0x100):
      try:
        ...
        print("Reboot the fucking device")
    ...
    while dev is None:
      time.sleep(5)
      dev = usb.core.find(idVendor=0x18d1, idProduct=0x9302)
      if loop == 5 and usb.core.find(idVendor=0x1a6e, idProduct=0x089a) is not None:
        raise Exception("Flash didn't work...if you are on windows try to set the backend correctly")
      loop+=1
  return dev
...
def run(dev, image_pixels):
  """
  classify our image based on the image pixels and imagenet lables
  """

  assert TF_MODEL == "inception_v4_299_quant_edgetpu.tflite" and  FW_BIN == "apex_latest_single_ep.bin", "YOU MIGHT NEED NEW OFFSETS FOR YOUR MODEL"

  # get labels
  lbls = open(LABELS).read().strip().split("\n")
  lbls = [x[6:] for x in lbls]

  # open model 
  et = open(TF_MODEL, "rb").read()
...
if __name__ == '__main__':
  if len(sys.argv)<2:
    raise Exception("Need an image url arg")

  # install firmware if we need to and return device
  dev = firmware_check()
  ...
  image_data = requests.get(sys.argv[1]).content
  ip = get_image_pixels(image_data)

  # run the image with the model
  run(dev, ip)
